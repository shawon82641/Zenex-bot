...
BOT_TOKEN = "8884952131:AAFCNNJOJpo1l2eox_VAMLlChpy5ncXkvnY"
API_KEY = "ZNX_NMQPYG5ZQC7KWYEP11EXIAXV"  # জেনেক্স এপিআই কী
...
def load_settings():
    if not os.path.exists(SETTINGS_FILE):
        default = {
            ...
            "otp_reward": 0.003,          # প্রতি ওটিপিতে বোনাস
            "refer_bonus": 0.050,          # রেফারেল বোনাস
            "numbers_per_request": 1       # একসাথে কয়টি নাম্বার দিবে
        }
    ...
    try:
        with open(SETTINGS_FILE, "r") as f:
            ...
            if "otp_reward" not in data:
                data["otp_reward"] = 0.003
                updated = True
            if "refer_bonus" not in data:
                data["refer_bonus"] = 0.050
                updated = True
            if "numbers_per_request" not in data:
                data["numbers_per_request"] = 1
                updated = True
            if "min_withdraw" not in data or data["min_withdraw"] == 20:
                data["min_withdraw"] = 0.2
                updated = True
            if updated:
                with open(SETTINGS_FILE, "w") as f:
        ...
        return {
            ...
            "force_join_channels": ["@instagramotp20"],
...
def get_withdraw_limits():
    settings = load_settings()
    return float(settings.get("min_withdraw", 0.2)), float(settings.get("max_withdraw", 100.0))
    ...
    for attempt in range(2):
        try:
            r = await client_async.get(
                f"{base_url}/v1/active-ranges",
                headers={"mapikey": api_key},
                timeout=httpx.Timeout(connect=4.0, read=10.0, write=4.0, pool=4.0)
            )
            data = r.json()

            # Zenex API v4 compatibility
            if isinstance(data, dict):
                if data.get("success") is True or data.get("meta", {}).get("status") == "success":
                    ranges_list = data.get("data", {}).get("active_ranges", [])
                elif "data" in data and isinstance(data.get("data"), dict) and data["data"].get("active_ranges"):
                    ranges_list = data["data"]["active_ranges"]
...
async def show_app_selection(update, context):
    ...
    if not top_ranges_by_app:
        await status.edit_text(
            "⚠️ Live range feed is empty.\n\n"
            "Zenex API is currently returning no active ranges. "
            "Please use a direct range or try again later."
        )
        return
...
async def support_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💬 SUPPORT", url=f"https://t.me/insotpgrambotowner", style="primary")],
...
async def monitor_loop(app):
    while True:
        try:
            api_key, base_url = get_api_credentials()
            r = await client_async.get(f"{base_url}/v1/numsuccess/info", headers={"mapikey": api_key})
            res = r.json()
            if "data" in res and "otps" in res["data"]:
                ...
                for otp in otps:
                    ...
                    if (num in active_numbers and 
                        ...
                        channel_url = settings.get("channel_url", "https://t.me/instagramotp20")
                        ...
                        user_otp_keyboard = InlineKeyboardMarkup([
                            ...
                            [
                                ...
                                InlineKeyboardButton("✔️ NUMBER ↗", url="http://t.me/otp_service2_bot", style="primary")
                            ]
                        ])
                        ...
                        group_buttons = InlineKeyboardMarkup([
                            [
                                InlineKeyboardButton("‼️ PANEL", url="https://t.me/otpgroupsl", style="danger"),
                                InlineKeyboardButton("📢 CHANNEL", url=channel_url, style="primary")
                            ]
                        ])
...
async def process_auto_number(update, context, range_text):
    ...
    try:
        ...
        settings = load_settings()
        otp_group_url = settings.get("otp_group_url", "https://t.me/otpgroupsl")
...
async def process_numbers(update, context, range_text, count, edit_message=None):
    ...
    try:
        ...
        settings = load_settings()
        otp_group_url = settings.get("otp_group_url", "https://t.me/otp_group_s")
...
async def hourly_traffic_broadcast_loop(app):
    ...
    while True:
        try:
            ...
            if top_ranges:
                lines = [
                    "📢 <b>OTP bot LIVE TRAFFIC UPDATE</b> 📢\n"
                    "━━━━━━━━━━━━━━━━━━━━━━\n\n"
                ...
                if has_traffic:
                    lines.append("\n━━━━━━━━━━━━━━━━━━━━━━\n🚀 <i>OTP Number bot | Lightning Fast Delivery</i>")
                    broadcast_text = "\n".join(lines)
...
async def admin_approve_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE, payment_id: str):
    ...
    user_final_msg = (
        ...
        "🚀 Otp service bot | SECURE & TRUSTED ⚡</blockquote>\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📊 SYS LIVE HEALTH" and is_admin(uid):
        ...
        try:
            api_key, base_url = get_api_credentials()
            r = await client_async.get(f"{base_url}/v1/active-ranges", headers={"mapikey": api_key}, timeout=5.0)
    ...
    if context.user_data.get("mode") == "input_user_id" and is_admin(uid):
        ...
        status_msg = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🚀 <b>Otp service bot | LIVE REAL-TIME DATA</b> ⚡"
        )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("withdraw_") and data not in ["withdraw_confirm", "withdraw_cancel"]:
    ...
    if data == "same_range":
        ...
        available_ranges = country_ranges[:15]
